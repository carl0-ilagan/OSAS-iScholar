...
export async function generateScholarshipCode(scholarshipName) {
  ...
  try {
    ...
    const isExpectedPermissionError = code.includes("permission-denied") || code.includes("failed-precondition")
    if (!isExpectedPermissionError) {
      console.error("Error generating scholarship code:", error)
    }
    // Fallback for restricted clients: generate a local unique suffix.
    const fallbackSuffix = `${Date.now().toString().slice(-4)}${Math.floor(Math.random() * 100)
      .toString()
      .padStart(2, "0")}`
    return `MINSU-${year}-${date}-${fallbackSuffix}`
  }
}
