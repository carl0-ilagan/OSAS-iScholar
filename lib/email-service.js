 ...
 * Handles sending emails for various actions in the MOCAS system
 */
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <p>Don't miss this opportunity! Log in to your MOCAS account to view details and apply now.</p>
          <p>Best regards,<br>MOCAS Team</p>
        </div>
      </div>
    </body>
...
export async function sendVerificationApprovedEmail(studentEmail, studentName) {
  const subject = 'Account Verification Approved - MOCAS'
...
export async function sendVerificationDeclinedEmail(studentEmail, studentName, reason) {
  const subject = 'Account Verification Update - MOCAS'
...
export async function sendApplicationSubmittedEmail(studentEmail, studentName, scholarshipName, trackerCode) {
  const subject = 'Application Submitted Successfully - MOCAS'
...
export async function sendApplicationApprovedEmail(studentEmail, studentName, scholarshipName) {
  const subject = '🎉 Application Approved - MOCAS'
...
export async function sendApplicationDeclinedEmail(studentEmail, studentName, scholarshipName, reason) {
  const subject = 'Application Status Update - MOCAS'
...
export async function sendTestimonialSubmittedEmail(studentEmail, studentName) {
  const subject = 'Testimonial Submitted - MOCAS'
...
export async function sendNewAnnouncementEmail(studentEmail, studentName, announcementTitle, announcementDescription) {
  const subject = '📢 New Announcement - MOCAS'
...
export async function sendNewScholarshipEmail(studentEmail, studentName, scholarshipName, scholarshipDescription) {
  const subject = '🎓 New Scholarship Available - MOCAS'
  ...
  return `
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          <p>Dear ${studentName},</p>
          <p>A new document requirement has been added to your MOCAS account:</p>
          ...
          <p>Please log in to your MOCAS account and go to the Requirements section to upload this document.</p>
          <p>Best regards,<br>MOCAS Team</p>
        </div>
      </div>
    </body>
...
export async function sendNewDocumentRequirementEmail(studentEmail, studentName, requirementName, requirementDescription, isRequired) {
  const subject = '📄 New Document Requirement - MOCAS'
  const html = getNewDocumentRequirementTemplate(studentName, requirementName, requirementDescription, isRequired)
  return await sendEmail(studentEmail, subject, html)
}

function getConsultationInviteTemplate(studentName, roomName, durationMinutes) {
  const durationText = durationMinutes ? `${durationMinutes} minutes` : "a scheduled session"
  return `
    <!DOCTYPE html>
    <html>
      <head>
        ...
        <style>
          ...
          .header { background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0; }
          .content { background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px; }
          .room-box { background: white; border: 2px solid #11998e; padding: 20px; margin: 20px 0; border-radius: 8px; }
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            <p>Dear ${studentName},</p>
            <p>You have been invited to a consultation on MOCAS.</p>
            <div class="room-box">
              <p class="muted" style="margin:0 0 6px 0;">Room</p>
              <p style="margin:0; font-size:18px; font-weight:bold; color:#11998e;">${roomName}</p>
              <p class="muted" style="margin:10px 0 0 0;">Duration: ${durationText}</p>
            </div>
            <p>Please log in to your MOCAS account to join the consultation when it is available.</p>
            <p>Best regards,<br/>MOCAS Team</p>
          </div>
        </div>
      </body>
...
function getConsultationJoinApprovedTemplate(studentName, roomName) {
  return `
    <!DOCTYPE html>
    <html>
      <head>
        ...
        <style>
          ...
          .content { background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px; }
          .success-box { background: #d4edda; border-left: 4px solid #28a745; padding: 15px; margin: 20px 0; }
          .room-box { background: white; border: 2px solid #667eea; padding: 20px; margin: 20px 0; border-radius: 8px; }
        </style>
      </head>
      <body>
        <div class="container">
          ...
          <div class="content">
            <p>Dear ${studentName},</p>
            <div class="success-box">
              <p style="margin:0;"><strong>Your request to join</strong> has been approved.</p>
            </div>
            <div class="room-box">
              <p class="muted" style="margin:0 0 6px 0; color:#666;">Room</p>
              <p style="margin:0; font-size:18px; font-weight:bold; color:#667eea;">${roomName}</p>
            </div>
            <p>You can now join the consultation from your MOCAS dashboard.</p>
            <p>Best regards,<br/>MOCAS Team</p>
          </div>
        </div>
      </body>
...
function getConsultationJoinRejectedTemplate(studentName, roomName) {
  return `
    <!DOCTYPE html>
    <html>
      <head>
        ...
        <style>
          ...
          .room-box { background: white; border: 2px solid #dc3545; padding: 20px; margin: 20px 0; border-radius: 8px; }
        </style>
      </head>
      <body>
        <div class="container">
          ...
          <div class="content">
            <p>Dear ${studentName},</p>
            <div class="error-box">
              <p style="margin:0;"><strong>Your request to join</strong> was not approved.</p>
            </div>
            <div class="room-box">
              <p class="muted" style="margin:0 0 6px 0; color:#666;">Room</p>
              <p style="margin:0; font-size:18px; font-weight:bold; color:#dc3545;">${roomName}</p>
            </div>
            <p>Please check back later or request again if allowed.</p>
            <p>Best regards,<br/>MOCAS Team</p>
          </div>
        </div>
      </body>
...
export async function sendConsultationInviteEmail(studentEmail, studentName, roomName, durationMinutes) {
  const subject = `🎥 Consultation Invitation - MOCAS`
  const html = getConsultationInviteTemplate(studentName, roomName, durationMinutes)
  return await sendEmail(studentEmail, subject, html)
}

export async function sendConsultationJoinApprovedEmail(studentEmail, studentName, roomName) {
  const subject = `✅ Join Request Approved - MOCAS`
  const html = getConsultationJoinApprovedTemplate(studentName, roomName)
  return await sendEmail(studentEmail, subject, html)
}

export async function sendConsultationJoinRejectedEmail(studentEmail, studentName, roomName) {
  const subject = `❌ Join Request Rejected - MOCAS`
  const html = getConsultationJoinRejectedTemplate(studentName, roomName)
  return await sendEmail(studentEmail, subject, html)
}
