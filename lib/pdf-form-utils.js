...
/**
 * Firestore does not support nested arrays (e.g. table grid = array of row arrays).
 * Serialize those (and plain objects) as prefixed JSON strings for storage.
 */
const SUBMISSION_VALUE_JSON_PREFIX = "__pdfFormJson:"

export function serializeSubmissionValueForFirestore(value) {
  if (value === null || value === undefined) return ""
  const t = typeof value
  if (t === "string" || t === "number" || t === "boolean") return value
  if (Array.isArray(value)) {
    if (value.some((item) => Array.isArray(item))) {
      return SUBMISSION_VALUE_JSON_PREFIX + JSON.stringify(value)
    }
    return value
  }
  if (t === "object") {
...
/** Restore values written with {@link serializeSubmissionValueForFirestore}. */
export function deserializeSubmissionValue(stored) {
  if (stored === null || stored === undefined) return stored
  if (typeof stored !== "string") return stored
  if (stored.startsWith(SUBMISSION_VALUE_JSON_PREFIX)) {
    try {
      return JSON.parse(stored.slice(SUBMISSION_VALUE_JSON_PREFIX.length))
    } catch {
      return stored
    }
  }
