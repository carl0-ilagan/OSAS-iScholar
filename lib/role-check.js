...
export const ADMIN_EMAILS = ["contact.ischolar@gmail.com", "mocas.support@gmail.com"]
...
export function isAdminEmail(email) {
  ...
  return ADMIN_EMAILS.some((adminEmail) => normalizedEmail === String(adminEmail).trim().toLowerCase())
}
...
export function isAdmin(user) {
  ...
  return role === ROLE_ADMIN || role === ROLE_CAMPUS_ADMIN || isAdminEmail(user.email)
}
...
export function isPrimaryAdmin(user) {
  ...
  const role = normalizeRole(user.appRole || user.role)
  return role === ROLE_ADMIN || isAdminEmail(user.email)
}
