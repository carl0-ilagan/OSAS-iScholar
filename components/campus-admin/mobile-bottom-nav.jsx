...
export default function CampusAdminMobileBottomNav() {
  ...
  const mobileNavItems = campusAdminNavItems.filter(
    (item) => item.href !== "/campus-admin/users" && item.href !== "/campus-admin/applications",
  )
        ...
        {mobileNavItems.map(({ icon: Icon, label, href }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className={`flex flex-1 flex-col items-center gap-0.5 rounded-xl px-1 py-1.5 ${
