...
export default function ApplicationFormPage() {
  ...
  const handleDownloadFilledPdf = async () => {
    try {
      ...
      const mergedData = { ...autoFillData, ...formData }
      if (!Object.keys(mergedData).length) {
        toast.error("Please fill up at least one field before downloading PDF")
        return
      }

      const response = await fetch("/api/application-form-pdf", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ formData: mergedData }),
      })
      ...
      link.href = url
      link.download = `application-form-${Date.now()}.pdf`
      ...
      toast.error(error?.message || "Unable to generate PDF right now")
    } finally {
